...
export const EventCard = (ev) => {
  if (ev.userId === parseInt(sessionStorage.getItem("activeUser")))
    return `<div class="eventCard"><p>${ev.name}</p> <p>${ev.eventCity}, ${ev.eventState}</p> <p>${ev.eventDate}</p><button class="editEvent--${ev.id}">edit</button></div>`
  else
...
export const EventCardFirst = (ev) => {
  if (ev.userId === parseInt(sessionStorage.getItem("activeUser")))
    return `<div class="eventCard nonOffensive"><p><b>${ev.name}</p> <p>${ev.eventCity}, ${ev.eventState}</p> <p>${ev.eventDate}</p></b><button class="editEvent--${ev.id}">edit</button></div>`
  else
